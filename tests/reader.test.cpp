...
TEST(Reader, StreamWithManifest) {
  ...
  const auto reader = c2pa::Reader("image/jpeg", file_stream);
  const auto manifest_store_json = reader.json();
  EXPECT_TRUE(manifest_store_json.find("C.jpg") != std::string::npos);
};

TEST(Reader, FileWithManifest) {
  // read the new manifest and display the JSON
  const auto reader = c2pa::Reader("../../tests/fixtures/C.jpg");
  const auto manifest_store_json = reader.json();
  EXPECT_TRUE(manifest_store_json.find("C.jpg") != std::string::npos);
};
...
TEST(Reader, FileNoManifest) {
  EXPECT_THROW(
      { auto reader = c2pa::Reader("../../tests/fixtures/A.jpg"); },
      c2pa::Exception);
};
