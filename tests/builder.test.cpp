...
TEST(Builder, SignFile) {
  ...
  try {
    ...
    // create a signer
    auto signer = c2pa::Signer(&test_signer, Es256, certs,
...
TEST(Builder, SignStream) {
  try {
    ...
    std::stringstream memory_buffer(std::ios::in | std::ios::out |
                                    std::ios::binary);
...
TEST(Builder, SignStreamCloudUrl) {
  try {
    ...
    // very important to use a URL that does not exist, otherwise you may get a
    // JumbfParseError or JumbfNotFound
    builder.set_remote_url("http://this_does_not_exist/foo.jpg");
    builder.set_no_embed();

    // auto manifest_data = builder.sign(signed_image_path, "target/dest.jpg",
    // signer);
    std::ifstream source(signed_image_path, std::ios::binary);
    if (!source) {
      FAIL() << "Failed to open file: " << signed_image_path << std::endl;
    ...
    if (std::string error_message = e.what();
        error_message.rfind("Remote ", 0) == 0) {
      SUCCEED();
    } else {
      FAIL() << "Failed: C2pa::Builder: " << e.what() << endl;
    }
...
TEST(Builder, SignDataHashedEmbedded) {
  try {
    ...
    const fs::path certs_path =
        current_dir / "../tests/fixtures/es256_certs.pem";
    // fs::path signed_image_path = current_dir / "../tests/fixtures/A.jpg";

    const auto manifest = read_text_file(manifest_path);
    const auto certs = read_text_file(certs_path);

    // create a signer
    const auto signer = c2pa::Signer(&test_signer, Es256, certs,
                                     "http://timestamp.digicert.com");

    const auto builder = c2pa::Builder(manifest);

    auto placeholder =
        builder.data_hashed_placeholder(signer.reserve_size(), "image/jpeg");

    const std::string data_hash = R"({
          "exclusions": [
            {
              "start": 20,
              "length": 45884
    ...
    auto manifest_data =
        builder.sign_data_hashed_embeddable(signer, data_hash, "image/jpeg");
  } catch (c2pa::Exception const &e) {
    FAIL() << "Failed: C2pa::Builder: " << e.what() << endl;
  };
}
...
TEST(Builder, SignDataHashedEmbeddedWithAsset) {
  try {
    ...
    auto embeddable_data =
        c2pa::Builder::format_embeddable("image/jpeg", manifest_data);
